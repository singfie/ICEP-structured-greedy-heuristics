...
def S_ICEP_heuristic(vessel_source, scenario_source, vessel_pos_source, 
	...
	current_cost = best_cost # initialize

	all_cost_evo.append(best_cost)
	best_cost_evo.append(best_cost)

	while (current_cost <= best_cost) and (len(resources) > 0):
	    ...
	    for a in resources:
	        if len(resources) >= 2 :
	            if len(resources) <= n-1:
	                resources.append(save_previous)
	                resources.remove(a) # remove the ath resource
	            else:
	                resources.remove(a)
	            ...
	            if proposed_cost < current_cost:
	            	print("Removing resource", a.name, "from the resources set reduces the objective function from", current_cost, 'to', proposed_cost)
	            	current_cost = proposed_cost
	            	best_route_set = route_plans # update
	            	best_evacuation_times = evacuation_times
	            	best_evacuee_numbers = not_evacuated
	            	save_previous = a # save the resource that was taken out
	             ...
	             #        not_in_resources.append(save_previous)
	            	...
	            	print("Removing resource", a.name, "tfrom the resources set does not reduce the objective function.")
...
def main():
	...
	for i in range(len(best_route_set)):
		...
		best_route_set[i].to_csv(os.path.join(path, 'solution/Greedy_S_ICEP_best_route_plan_scenario_backwards_') + str(i+1) + ".csv")

	# write a performance file
	performance_metrics = open(os.path.join(path, "solution/Greedy_S_ICEP_solution_metrics_backwards.txt"),"w+")
